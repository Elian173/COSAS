...
int pantalla_mostrar(Pantalla* array,int limite)
{
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        {
            if(!array[i].isEmpty)
                printf("[RELEASE] - %d - %s -%.2f - %d\n",array[i].idPantalla, array[i].nombre,array[i].precio, array[i].isEmpty);
        }
    }
    return retorno;
}
...
int pantalla_ordenarAlfabeticamente(Pantalla *array,int limite, int orden)
{
    int retorno = -1;
    int i;
    int flagSwap;
...
int pantalla_mostrarAPartirDe(Pantalla *array, int limite,int minOmax, int minLimite)
{
    ...
    for(i=0; i<limite; i++)
    {
        ...
        if(limite > 0 && array != NULL)
        {
            if((array[i].precio<=minLimite && !minOmax) || (array[i].precio>=minLimite && minOmax))
            {
                retorno = 0;
                if(!array[i].isEmpty)
                    printf("[RELEASE] - %d - %s -%.2f - %d\n",array[i].idPantalla, array[i].nombre,array[i].precio, array[i].isEmpty);
...
int pantalla_calcularPromedio(Pantalla *array, int limite, float *promedioPantallas)
{
    ...
    if(limite > 0 && array != NULL)
    {
        for(i=0; i<limite; i++)
        {
            if(!array[i].isEmpty)
            {
                acumuladorPrecios+=array[i].precio;
                contadorPantallas++;
            }
        }
        promedio=acumuladorPrecios/contadorPantallas;
        printf("---- %f\n\n", promedio);
       *promedioPantallas=promedio;
       printf("%f  ---- %d",acumuladorPrecios, contadorPantallas);
    }
    return retorno;
}
